...
QUERY = obj(schema_version={"const": 1}, record_type={"const": "market_query"}, provider={"const": "alpaca"},
...
def validate(value):
    try:
        ...
        if value["record_type"] == "market_query":
            ...
            if any(not value["start"][:10] <= day <= value["end"][:10] for day in value["expected_sessions"]):
