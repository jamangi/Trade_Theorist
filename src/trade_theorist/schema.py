...
FIELDS = {
    ...
    "source": dict(title=S, authors=array(S, 1), edition=S, isbn=nullable({"type": "string", "pattern": "^[0-9]{13}$"}), publisher=S, locator=S, retrieved_at=UTC, checked_at=UTC, next_check_at=UTC, access=enum("unknown", "public_full_text", "sample_only", "library_loan", "owned_copy", "purchase_required", "unavailable"), rights=RIGHTS, publication_eligibility=enum("unknown", "metadata_only", "derived_permitted", "raw_permitted"), private_locator=nullable(S), ingestion_status=enum("not_started", "partial", "complete", "blocked"), blocker=nullable(S)),
