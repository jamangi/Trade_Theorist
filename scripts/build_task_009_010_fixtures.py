...
class FixtureScenario:
    def __init__(self, store, setup, export_root, provider=None):
        ...
        self.council = Council(store, setup["conversation"]["id"])
    ...
    def operations(self):
        ...
        def opinions(_store, _context, external):
            commits = [council.commit_initial(item["recommendation_id"], at=OPINION_AT)
        ...
        def decide(_store, context, _external):
            ...
            commit = council.commit_final(setup["final"]["id"], at=FINAL_AT, considered_message_ids=considered)
