...
This repository currently covers **Trade Theorist only**. It retrieves permitted market data for private research and simulated portfolios. It does not place broker orders or promise profitable results.

## Start here: the research observatory

**Proposed next architecture:** the [redesign comparison](redesign/README.md) explains current Character learning and compares a fresh theory-learning laboratory with a bounded adaptation. These are proposals; the operating status and gates below remain in force.

**Current status (2026-09-15 UTC): Step 12's readiness audit is complete; paper promotion is held at forward shadow.** Read the [audit decision](docs/step-12-readiness-audit.md), [selected policy](examples/step-12/paper-policy-decision.json) and [validation evidence](examples/step-12/validation.json). All 35 saved accounting results and seven immutable reports verified. The five-session, one-forecast trial remains below the 60/30 review floors; Step 13 is blocked. Both finite observation tasks are disabled with receipts preserved. The [original handoff](docs/step-12-audit-handoff.md) locates the private evidence. Index Steward read Bogle; Value Rationalist completed Graham through 28 units; Systematic Trend Operator completed Faith through 17. Each has a scoped eligible version, while curriculum positions 2–4 and theory evaluation remain pending. Historical partial records are preserved.

Step 09 qualified private delayed daily SIP bars. Step 11 added real dividend/split qualification and a tools-free prospective path with immutable learning ancestry, three independent opinions, bounded Index-led council advice and matched simulated portfolios. The initial runtime uses fixed foundation-informed rules, not new LLM deliberation or validated strategies. The September 8–14 sessions were collected by the September 14, 7:30 p.m. Chicago Windows task: five completed sessions, one matured forecast and no source gaps. [Completion evidence](examples/step-11/observation-completion.json) records the verified result. [Steps 11A/11B](docs/step-11-automation.md) now provide private start/end logs and a rehearsed finite Windows observation schedule: September 14 at 7:30 p.m. Chicago, fallback September 15 at 8 a.m., and a noon deadline check. The owner must remain signed into Windows; Codex need not be open. Zero broker orders or external model calls. [Step 11C](docs/step-11-private-backups.md) now has a verified private local backup and offline restore, preserving spent quotas and reconciling stored accounting. [Remote recovery](docs/step-11-remote-recovery.md) is now verified on the owner's Lightsail server, including independent PIN-and-touch recovery with both FIDO keys; general recurrence stays in Step 15. Observed forward results are available; Step 12 records a stage hold and delegated numeric policy. Paper execution remains blocked by its evidence floors and future-run engineering requirements.

Start with the [Step 11 guide](docs/step-11-forward-observation.md), [actual run status](examples/step-11/forward-status.json), [readiness register](examples/step-11/readiness-register.v2.json), and [ordered task queue](tasks/README.md). Run the [offline demo](docs/quickstart.md) after setup. The [inventory](library/catalog/INVENTORY_REPORT.md) covers all seven Characters. Source PDFs and real market data stay local; public artifacts contain metadata, original analysis, operational counts and hashes. Existing fixtures retain their synthetic evidence labels.
...
[META-001 is complete](docs/meta-001-impact.md). It adopts a private Individual/Monarchy UI, versioned accounting and opaque Monarchy-only broker attribution. The legacy exporter refuses nonfixture data. [Step 01: contracts and storage](tasks/active/STEP-01-contracts.md) and [Step 02: accounting](tasks/active/STEP-02-accounting.md) are implemented and verified: explicit v2 persistence, a checked synthetic migration, production FIFO/flow-adjusted returns and offline broker reconciliation preserve v1 results. See the [accounting guide and evidence](docs/step-02-accounting.md). [Step 03: private dashboard](docs/step-03-private-dashboard.md) now adds the verified Individual/Monarchy views, with a distinct private read model and preserved fixture UI. [Step 04: private commands](docs/step-04-commands.md) now adds explicit v1/v2 selection, read-only diagnostics, resumable private evaluation/export and a clean offline installation check. [Step 05: protected local packaging](docs/step-05-local-package.md) now adds a validated bundle manifest, protected loopback launch/stop and conservative retention. [Step 06: shared requests](docs/step-06-shared-requests.md) adds durable admission, full cooldowns, cache/batching, bound resume and offline collector diagnostics. [Step 07: forward integration](docs/step-07-forward-integration.md) adds preregistered paired rounds, equal immutable snapshots, separate finite budgets and truthful offline abstention. [Step 08: independent preflight](docs/step-08-offline-preflight.md) verifies transport timing and process recovery, resolves four defects, and preserves dated baseline/final evidence. [Step 09: vendor qualification](docs/step-09-vendor-qualification.md) is complete for private delayed daily SIP, with standing owner authorization, six actual requests, complete coverage and zero duplicate additions on restored replay. Its compact guide links the public operational evidence and private-store continuation instructions. [Step 10: participant readiness](docs/step-10-pilot-readiness.md) completes the cited review and reconciles an eligible Index version for its limited Bogle-grounded role. Value and Trend remain unready; their remaining foundations block the three-participant gate and Step 11. Those historical learning blockers were resolved in Step 11; the [numbered execution sequence](tasks/README.md) now records completed Step 12 and blocked Step 13 after verified outcome collection. Start from those repository guides in a fresh task; no conversation history is required. Steps 06–13 separately cover shared requests, offline forward integration, preflight, qualification, participant readiness, real observation, final audit and paper operation. Step 09 supersedes its earlier blocked review; Step 10 review is complete with a blocked participant gate; Steps 11–12 are now complete, while Step 13 remains blocked by the audit decision; [the crosswalk](tasks/CROSSWALK.md) preserves original task identities and completed v1 evidence.
