...
**Current status (2026-09-07 UTC): all three pilot foundations are complete, and Step 11's finite forward shadow window is in progress.** Index Steward read Bogle; Value Rationalist completed Graham through 28 units; Systematic Trend Operator completed Faith through 17. Each has a scoped eligible version, while curriculum positions 2–4 and theory evaluation remain pending. Historical partial records are preserved.

Step 09 qualified private delayed daily SIP bars. Step 11 added real dividend/split qualification and a tools-free prospective path with immutable learning ancestry, three independent opinions, bounded Index-led council advice and matched simulated portfolios. The initial runtime uses fixed foundation-informed rules, not new LLM deliberation or validated strategies. Future sessions are September 8–14. [Steps 11A/11B](docs/step-11-automation.md) now provide private start/end logs and a rehearsed finite Windows observation schedule: September 14 at 7:30 p.m. Chicago, fallback September 15 at 8 a.m., and a noon deadline check. The owner must remain signed into Windows; Codex need not be open. Zero broker orders or external model calls. [Step 11C](docs/step-11-private-backups.md) now has a verified private local backup and offline restore, preserving spent quotas and reconciling stored accounting. [Remote recovery](docs/step-11-remote-recovery.md) is now verified on the owner's Lightsail server, including independent PIN-and-touch recovery with both FIDO keys; general recurrence stays in Step 15. No completed forward performance or paper-stage approval is claimed.
